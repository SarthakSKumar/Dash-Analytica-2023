...
for placemark in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
    pscode = placemark.find('.//{http://www.opengis.net/kml/2.2}SimpleData[@name="PSCode"]')
    if pscode is not None and int(pscode.text.strip()) in blacklist:
        root.remove(placemark)


tree.write('final.kml', xml_declaration=True, encoding='UTF-8')
